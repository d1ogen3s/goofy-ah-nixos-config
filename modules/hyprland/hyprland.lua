...
-- Fixes flickering in Electron apps
...
-- Helper function to load pywal colors
local function get_wal_colors()
	...
	if file then
		for line in file:lines() do
			-- Find the hex code (ignores the #)
			local hex = line:match("#(%w+)")
			if hex then
				-- Formats to rgba(XXXXXXff) so opacity is 100%
				table.insert(colors, "rgba(" .. hex .. "ff)")
			end
		end
		file:close()
	else
		-- Fallback colors just in case the file doesn't exist yet
		for i = 1, 16 do
			table.insert(colors, "rgba(909090ff)")
		end
	end
...
hl.config({
	general = {
		...
		col = {
			-- wal[5] and wal[7] (colors 4 and 6 from pywal) usually make a nice gradient
			active_border = "#0096FF",

			-- wal[1] is the dark background color (color0)
			inactive_border = wal[1],
		},
