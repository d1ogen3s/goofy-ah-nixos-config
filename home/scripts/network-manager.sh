...
manage_wifi() {
	...
	local chosen_network=$(echo -e "$formatted_list" | rofi -case-smart -dmenu -i -selected-row 1 -p "Wi-Fi SSID: ")
	local ssid_index=-1
	...
	if [ -z "$chosen_network" ]; then
		...
		action=$(echo -e "$action\n  Forget" | rofi -case-smart -dmenu -p "Action: ")
		case $action in
			"󰸋  Connect")
				...
				if [[ $(echo "$saved_connections" | grep -Fx "$chosen_id") ]]; then
					...
					local wifi_password=$(rofi -case-smart -dmenu -p "Password: " -password)
...
manage_ethernet() {
	...
	local chosen_device=$(echo -e "$eth_list" | rofi -case-smart -dmenu -i -p "Select Ethernet device: ")
...
main_menu() {
	...
	##==> Выводим Rofi меню
	#######################################################
	local chosen_option=$(echo -e "$wifi_toggle$manage_wifi_btn\n󱓥 Manage Ethernet" | rofi -case-smart -dmenu -p " Network Management: ")
...
do
	case "$1" in
		--only-wifi) manage_wifi "$@"
			;;
		--only-ethernet) manage_ethernet "$@"
			;;
		*) echo "bad option $1"
			;;
	esac
